...
def PROCEDURE_MAINGAME():
    ...
    while True:
        ...
        if len(spiders) < maxspiders and access_level >= 2 and alive:
            if True:#random.randint(1, 120) == 1:
            ...
            if keys[pygame.K_c] and reload[2] == 0:
                ...
                for item in spiders[:]:
                    ...
                    if totalDistance <= bigrad:
                        spiders.remove(item)
                SWORD.play()
        print(player_coords[0], 800-player_coords[1])
        for item in spiders[:]:
            AX, AY = item[0], item[1]
            BX, BY = player_coords[0]+PLAYERWIDTH/2, player_coords[1]+PLAYERHEIGHT/2
        ...
        if access_level >= 2:
            ...
            pygame.draw.circle(SCREEN, COLORSRGBL[avatar_no], (player_coords[0]+PLAYERWIDTH/2, player_coords[1]+PLAYERHEIGHT/2+offset), smallrad, 5)
            ...
            if access_level == 3:
                ...
                pygame.draw.circle(SCREEN, COLORSRGB[avatar_no], (player_coords[0]+PLAYERWIDTH/2, player_coords[1]+PLAYERHEIGHT/2+offset), bigrad, 5)
        for item in spiders[:]:
            AX, AY = item[0], item[1]
            BX, BY = player_coords[0]+PLAYERWIDTH/2, player_coords[1]+PLAYERHEIGHT/2
            totalDistance = numpy.sqrt(numpy.square(AX-BX)+numpy.square(AY-BY))
