...
def PROCEDURE_MAINGAME():
    ...
    while True:
        ...
        for item in lava:
            if True:
                if alive and item[0] < player_coords[0]+PLAYERWIDTH/2 < item[0]+BLOCKWIDTH and item[1] < player_coords[1]+PLAYERHEIGHT <= item[1]+BLOCKWIDTH:
                    HP -= 1
        for item in blocks:
            if True:
        ...
        if keys[pygame.K_a] and lr_movement[0] and alive:
            player_coords[0] -= 2
        if keys[pygame.K_d] and lr_movement[1] and alive:
            player_coords[0] += 2
