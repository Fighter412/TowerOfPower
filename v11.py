...
def PROCEDURE_MAINGAME():
    ...
    while True:
        ...
        if len(spiders) < maxspiders and access_level >= 2:
            spiders.append([random.randint(int(player_coords[0]+PLAYERWIDTH/2-radius), int(player_coords[0]+PLAYERWIDTH/2+radius)), random.randint(int(player_coords[1]+PLAYERHEIGHT/2-radius), int(player_coords[1]+PLAYERHEIGHT/2+radius))])
        if HP == 0:
            alive = False
