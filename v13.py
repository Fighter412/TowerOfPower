...
END = pygame.image.load(r"Assets\Others\Screenshot 2024-05-15 183913.png")
TRACK = pygame.mixer.music.load(r"Sounds\track.mp3")
...
HW = pygame.mixer.Sound(r"Sounds\halal.mp3")
...
BLACK = (0, 0, 0)
WHITE = (255, 255, 255)
COLORSRGB = [(0,152,220), (51,152,75), (139,172,15), (234,50,60), (191,111,74), (200,145,62), (255,173,99), (255,219,162)]
COLORSRGBL = [(167,228,255), (186,231,196), (230,247,165), (248,193,196), (235,15,92), (238,221,197), (255,230,208), (255,244,227)]   
...
def PROCEDURE_MAINGAME():
    ...
    while True:
        ...
        if access_level == 0 and player_coords[1] <= 350:
            ...
            maxspiders = 5
            blitted = False
        elif access_level == 2 and player_coords[1] <= -3150 and 900<=player_coords[0]<=1000:
            access_level = 3
            maxspiders = 50
            blitted = False
            value = PROCEDURE_CONTINUE()
            player_coords = [916, -3300]
        if len(spiders) < maxspiders and access_level >= 2 and alive:
            if random.randint(1, 20) == 1:
                x = random.randint(int(player_coords[0]+PLAYERWIDTH/2-radius), int(player_coords[0]+PLAYERWIDTH/2+radius))
                y = random.randint(int(player_coords[1]+PLAYERHEIGHT/2-2*radius), int(player_coords[1]+PLAYERHEIGHT/2))
                if x*x + y*y >= 300:
                    spiders.append([x ,y])
        if frame == FRAMERATE * 1:
            ...
            WALL.blit(ADVA, (1150, 800-5000+VALUE))
            blitted = True
        '''if player_coords[1] + offset < 500:
            offset += 5
        elif player_coords[1] + offset > 600:
            offset -= 5'''
        offset = 550- player_coords[1]
        ...
        if not alternative and access_level == 3:
            if keys[pygame.K_q] and reload[1] == 0:
                ...
                for item in spiders[:]:
                    ...
                    BX, BY = player_coords[0]+PLAYERWIDTH/2, player_coords[1]+PLAYERHEIGHT/2
                    totalDistance = numpy.sqrt(numpy.square(AX-BX)+numpy.square(AY-BY))
                    if totalDistance <= bigrad and AX<BX:
                        spiders.remove(item)
                SWORD.play()
        ...
        for item in spiders[:]:
            AX, AY = item[0], item[1]
            BX, BY = player_coords[0]+PLAYERWIDTH/2, player_coords[1]+PLAYERHEIGHT/2
            pygame.draw.rect(SCREEN, (0, 0, 0), pygame.Rect(AX, AY+offset, 10, 10))
            pygame.draw.rect(SCREEN, (0, 0, 0), pygame.Rect(BX, BY+offset, 10, 10))
        ...
        #pygame.draw.rect(SCREEN, RED, pygame.Rect(player_coords[0], player_coords[1]+offset, PLAYERWIDTH, PLAYERHEIGHT), 5)
        if not alternative:
        ...
        if access_level >= 2:
            ...
            pygame.draw.circle(SCREEN, COLORSRGBL[avatar_no], (player_coords[0]+PLAYERWIDTH/2, player_coords[1]+PLAYERHEIGHT/2+offset), smallrad, 2)
            ...
            if access_level == 3:
                ...
                pygame.draw.circle(SCREEN, COLORSRGB[avatar_no], (player_coords[0]+PLAYERWIDTH/2, player_coords[1]+PLAYERHEIGHT/2+offset), bigrad, 3)
        for item in spiders[:]:
            AX, AY = item[0], item[1]
            BX, BY = player_coords[0]+PLAYERWIDTH/2, player_coords[1]+PLAYERHEIGHT/2
            totalDistance = numpy.sqrt(numpy.square(AX-BX)+numpy.square(AY-BY))
            if totalDistance <= 20 and alive:
                HP -= 10
                DMG.play()
                spiders.remove(item)
            elif totalDistance >= 700:
                spiders.remove(item)
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN: 
                mouse = pygame.mouse.get_pos()
                print(mouse)
                if 756 <= mouse[0] <= 756+382 and 660 <= mouse[1] <= 660+128:
                    HW.stop()
